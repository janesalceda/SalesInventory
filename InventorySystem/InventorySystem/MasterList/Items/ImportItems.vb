        ...
        Try
            If MsgBox("Are you sure you want to save?", vbQuestion + vbYesNo, "Confirmation") = vbYes Then
                ...
                For i As Integer = 0 To DataGridView1.Rows.Count - 1
                    ...
                    VALUES 
	                    ( (select 'IT'+replace(convert(date,GETDATE()),'-','') +'-' + 
	                        CASE WHEN num<10 THEN '0' + CAST(num AS VARCHAR)    
                            ELSE CAST(num AS VARCHAR) END 'num' from
                            (SELECT(select count(*) from items where convert(date,createddate)=convert(date,getdate())) +
                            (select count(*) from ItemsForApproval where convert(date,createddate)=convert(date,getdate()))
                            'num')a),@cliapplieddate,@cliunitprice,NULL,@updatedby)")
                    SQL.AddParams("@qty", DataGridView1.Rows(i).Cells(16).Value)
                    SQL.AddParams("@ClientUnitprice", DataGridView1.Rows(i).Cells(8).Value)
                    SQL.AddParams("@SupplierUnitprice", DataGridView1.Rows(i).Cells(8).Value)
                    SQL.AddParams("@remarks", DataGridView1.Rows(i).Cells(7).Value)
                    SQL.AddParams("@updatedby", moduleId)
                    SQL.ExecQuery("INSERT INTO dbo.StockTakingDetails
                        ...
                        @remarks,
                        @updatedby)")
                    totalamount = DataGridView1.Rows(i).Cells(16).Value * DataGridView1.Rows(i).Cells(8).Value
                    ...
                    If SQL.HasException Then
                        msgboxDisplay("Error in saving StockTaking", 3)
                        SQL.ExecQuery("DELETE FROM StocktakingDETAILS WHERE CreatedDate>=(
                        SELECT CreatedDate FROM StocktakingDETAILS WHERE ITEMID='" & strItems & "')")
                        SQL.ExecQuery("DELETE FROM ClientItemPrices WHERE CreatedDate>=(
                        SELECT CreatedDate FROM StocktakingDETAILS WHERE ITEMID='" & strItems & "')")
                        SQL.ExecQuery("DELETE FROM SupplierItemPrices WHERE CreatedDate>=(
                        SELECT CreatedDate FROM StocktakingDETAILS WHERE ITEMID='" & strItems & "')")
                ...
                SQL.AddParams("@totalamount", totalamount)
                SQL.ExecQuery("INSERT INTO dbo.StockTakingHeaders
	                    (STID,CountedDate,EncodedStaff,TotalAmount,Remarks,ApprovedBy,UpdatedBy)
                                VALUES((select 'ST'+replace(convert(date,GETDATE()),'-','') +'-'+ NUM AS 'pomax' FROM
                                (select CASE WHEN (count(*)+1)<10 THEN '0' + CAST(COUNT(*)+1 AS VARCHAR) 
                                ELSE CAST(COUNT(*)+1 AS VARCHAR) END 'num' 
                                from StockTakingHeaders where convert(date,createddate)=convert(date,getdate()))A),getdate(),@encodedstaff,@totalamount,'',NULL,@encodedstaff)")
                If SQL.HasException Then
                    Exit Sub
                End If
